				...
				<div class="row">
					...
					<div class="col-lg-12">
						<div class="contact-us-form">
								...
								<div class="row">
									<div class="col-lg-12">
										<div class="form-group">
											<input type="date" name="date" required="" id="reservationDate">
											<div class="error-message" id="dateError"></div>
										</div>
									</div>
									<div class="col-lg-12">
										<div class="form-group">
											<input type="time" name="time" id="appointment-time" required="" min="09:00" max="16:00">
									...
									<div class="col-lg-12">
										<div class="form-group login-btn">
											<button class="btn" name="btn" type="submit" id="reserveButton" disabled>RESERVE</button>
										</div>
									</div>
							...
							<?php
							include '../../Model/appoinment.php';
							include '../../Controller/appoinmentC.php';
							include '../../Controller/config.php';
							if (isset($_POST['btn'])) {
								$role = isset($_SESSION['role']) ? $_SESSION['role'] : '';
								if ($role == 'admin' || $role == 'doctor' || $role == 'pharmacy') {
									echo '<div style="color: red;" class="alert alert-danger  mb-4 col-lg-12">only for patient!</div>';
								} else {
									$id_p = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : '';
									if ($id_p == '') {
										echo '<div style="color: red;" class="alert alert-danger  mb-4 col-lg-12">U Need To Connect!</div>';
									} else {
										$date = $_POST['date'];
										$time = $_POST['time'];
										$id_d = $_GET['id'];
										$controller = new apppoinementC();
										$res = $controller->checkDuplicateRdv($date, $time, $id_d);
										if ($res) {
											$rdv = new appoinement($date, $time, $id_p, $id_d, "en attente");
											$controller->addRdv($rdv);
											echo '<script>window.location.href = "doctors.php";</script>';
										} else {
											echo '<div style="color: red;" class="alert alert-danger  mb-4 col-lg-12">THIS TIME ALREADY EXIST!</div>';
										}
									}
								}
							}
							...
							<script>
								...
								function enableSubmitButton() {
									...
									var reservationDateValue = document.getElementById('reservationDate').value.trim();
									var timeValue = document.getElementById('appointment-time').value.trim();

									var dateIsValid = reservationDateValue !== '' && new Date(reservationDateValue) >= today;
									var timeIsValid = timeValue !== '' && timeValue >= '09:00' && timeValue <= '16:00';

									var reserveButton = document.getElementById('reserveButton');
									reserveButton.disabled = !(dateIsValid && timeIsValid);

									return false;
								}

								document.getElementById('reservationDate').addEventListener('input', function() {
									validateInput('reservationDate', 'dateError', 'Veuillez sélectionner une date');
								});

								document.getElementById('appointment-time').addEventListener('input', function() {
									validateInput('appointment-time', 'timeError', 'Veuillez sélectionner une heure de rendez-vous');
								});

								document.getElementById('reservationForm').addEventListener('submit', enableSubmitButton);
							</script>
