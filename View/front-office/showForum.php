...
<head>
	...
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" integrity="sha512-dNm0a6TZpPGK7gWJkVjFS1hZpI5dl4JZpe0QGi0voXOB2D30ODZNTGGNxFUy1A9N" crossorigin="anonymous" />
				...
				if (is_array($result) && !empty($result)) {
					...
					<a href='commenter.php?id_forum={$row['id_forum']}'>
						<div class='single-news'>
							<div class='news-head'>
								<a href='commenter.php?id_forum={$row['id_forum']}'><img src='../../src/{$row['img']}' alt='#' style='max-width: 300px; height: 300px;'> </a>
								<hr style=' color:blue;'>
							</div>
							<div class='news-body'>
								<div class='news-content'>
									<span > <span style='color: #3498db; font-size: 16px;'> Name:</span> {$row['auteur']}</span> <br>
									<span><span style='color: #3498db; font-size: 16px;'>Forum's Title: </span><a href='commenter.php?id_forum={$row['id_forum']}'>{$row['titre']}</a></span> <br>
						...
						<form method='GET' action='likes.php'>
            <input type='hidden' name='id_forum' value='{$row['id_forum']}'>
            
            <button type='submit'  name='btn_like'  style='color: black; background-color:white; width: 40px; height: 20px; border: 1px solid black; box-shadow: 6px 6px 5px #888;border-radius: 5px;vertical-align: middle;' >
                <i class='fa fa-thumbs-up fa-lg'></i>
				<span class='likes-count'> {$likee->countLikes($row['id_forum'])}</span>
            </button>

			<button type='submit' name='btn_dislike'  style='color: black; background-color:white ;width: 40px; height: 20px; border: 1px solid black; box-shadow: 6px 6px 5px #888;border-radius: 5px;vertical-align: middle;'>
                <i class='fa fa-thumbs-down fa-lg'></i>
				<span class='likes-count'> {$dislikee->countdisLikes($row['id_forum'])}</span>
            </button>

			<a  href='commenter.php?id_forum={$row['id_forum']}'   style='color: black; background-color: white; width: 40px; height: 20px; border: 1px solid black; box-shadow: 6px 6px 5px #888; display: inline-block; text-decoration: none;border-radius: 5px;vertical-align: middle;'>
				<center >
			<i class='fa fa-comment' ></i>
				<span class='likes-count'> {$cmnn->countcmn($row['id_forum'])}</span>
				</center>
            </a>
