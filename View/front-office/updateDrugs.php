...
$id_med = $_GET["id"];
$newread = new MedicamentC();
$result = $newread->getMedicamentById($id_med);
$allcat =new qtc() ;
$res =$allcat->read_catg($id_ph);
...
if (isset($_POST['btn_add'])) {
  ...
  $upDrug = new Medicament($nom, $description, $prix, $qte_med,$id_catg , $img, $id_ph);

  $update = $newread->updateMedicament($upDrug, $id_med);
  if ($update == 1) {
        ...
        <div class="row">
          <div class="col-12">
            ...
            <ul class="bread-list">
              ...
              <li class="active">Update drugs</li>
            </ul>
          </div>
        </div>
      </div>
                ...
                <div class="row">
                  <div class="col-lg-6">
                    <div class="form-group">
                      <input type="text" name="name" placeholder="name" required="required" value="<?php foreach ($result as $row) {
                                                                                                      echo $row['nom'];
                                                                                                    }   ?>" />
                    </div>
                  </div>
                  <div class="col-lg-6">
                    <div class="form-group">
                      <input type="text" name="description" placeholder="description" required="required" value="<?php foreach ($result as $row) {
                  ...
                  <div class="col-lg-6">
                    <div class="form-group">
                      <input type="text" name="price" placeholder="price" required="required" value="<?php foreach ($result as $row) {
                                                                                                        echo $row['prix'];
                                                                                                      } ?>" />
                    </div>
                  </div>
                  ...
                  <div class="col-lg-6">
                    <div class="form-group">
                      <select name="category" id="">
                       <option value=" <?php echo $result[0]['id_catg']  ?>"><?php $c=new qtc(); 
                       $category =$c->getcatById($result[0]['id_catg']);
                       echo $category[0]['nom'] ;?></option>
                       
                       <?php foreach ($res as $row) {
                        if ($row['nom'] != $category[0]['nom']) {
                          echo"<option value='{$row['id_catg']}'>{$row['nom']}</option>" ;
                        }
                       } ?>
                  ...
                  <div class="col-lg-6">
                    <div class="form-group">
                      <input type="text" name="qte" placeholder="quantity" required="required" value="<?php foreach ($result as $row) {
                                                                                                        echo $row['qte_med'];
                                                                                                      } ?>" />
                    </div>
                  </div>
                  <div class="col-lg-6">
                    <div class="form-group">
                      <input type="file" class="custom-file-input" name="file" required="" id="fileInput" style="display: none" value="<?php foreach ($result as $row) {
                                                                                                                                          echo $row['img'];
                                                                                                                                        } ?>" />
                  ...
                  <div class="col-lg-6">
                    <div class="form-group login-btn">
                      <button name="btn_add" class="btn" type="submit">UPdate drugs</button>
                    </div>
                  </div>
                  <div class="col-lg-3"></div>
                </div>
